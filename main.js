...
class GameManager {
    ...
    initDOM() {
        ...
        this.loopVideo = document.getElementById('loop-video'); // Warning
        this.triggerVideo = document.getElementById('trigger-video'); // Events
    ...
    initEvents() {
        ...
        // Custom Rendering Loop for Clipped Circles
    ...
    renderCustomFruits() {
        ...
        bodies.forEach(body => {
            if (body.label.startsWith('fruit')) {
                ...
                // 【修正】drawImage のガード強化（厳密な型チェックとロード完了確認）
                if (!img || !(img instanceof HTMLImageElement) || !img.complete) {
                    // Fallback if image not ready or invalid
                    ctx.beginPath();
    ...
    attemptDrop() {
        ...
        this.canDrop = false;
        this.mergeCount = 0; // Reset combo count per drop
        ...
        // Note: render.visible = false so we can draw it manually in afterRender
        ...
        body.fruitIndex = idx;
        body.circleRadius = r; // Explicit ref
    ...
    drawGuide() {
        ...
        if (this.images[idx] && this.images[idx].complete && (this.images[idx] instanceof HTMLImageElement)) {
            const img = this.images[idx];
            ctx.save();
            ctx.globalAlpha = 0.6;
            ctx.translate(x, y);
    ...
    merge(a, b) {
        ...
        // Check Kiriban (Crossed 1000 threshold)
        ...
        if (next < FRUITS.length) {
            ...
            // Check Suika Birth (Melon(9) + Melon(9) -> Suika(10))
            if (next === 10) { // Index 10 is watermelon
            ...
            const nb = Bodies.circle(mx, my, r, {
                label: `fruit_${next}`,
                restitution: 0.2, friction: 0.3,
                render: { visible: false } // Manual render
            });
            nb.fruitIndex = next;
            nb.circleRadius = r;
    ...
    updateLoop() {
        ...
        if (danger) {
            ...
            // Start Warning Video Loop if not playing
    ...
    gameOver() {
        ...
        // Game Over Video (Blocking)
        // Stop warning loop first
        this.loopVideo.pause();
        this.loopVideo.classList.add('hidden');
    ...
    playTriggerVideo(name, callback) {
        ...
        this.triggerVideo.play().catch(() => {
            // If autoplay blocked, force finish
            this.triggerVideo.classList.add('hidden');
            if (this.triggerCallback) this.triggerCallback();
        });
    }
}
