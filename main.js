    ...
    updateLoop() {
        ...
        if (danger) {
            ...
            if (!this.warningPlaying) {
                ...
                // 動画再生開始時にBGMが止まるのを防ぐため、明示的に再生を継続させる (Force Resume)
                this.loopVideo.play().then(() => {
                    if (this.bgmPlaying && this.bgm.paused) this.bgm.play().catch(() => { });
                }).catch(() => { });
